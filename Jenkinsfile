...
node {
    ...
    ws("${gopath}/src/github.com/ONSdigital/dp-dd-frontend-controller") {
        stage('Checkout') {
            checkout scm
            sh 'git clean -dfx'
        ...
        stage('Build') {
            sh "GOPATH=${gopath} go build -o build/dp-dd-frontend-controller"
        }
        ...
        stage('Image') {
            docker.withRegistry("https://${env.ECR_REPOSITORY_URI}", { ->
                ...
                docker.build('dp-dd-frontend-controller', '--no-cache --pull --rm .').push(revision)
            })
        }
        ...
        stage('Bundle') {
            ...
            sh "tar -cvzf dp-dd-frontend-controller-${revision}.tar.gz appspec.yml scripts/codedeploy"
            sh "aws s3 cp dp-dd-frontend-controller-${revision}.tar.gz s3://${env.S3_REVISIONS_BUCKET}/"
        }
        ...
        stage('Deploy') {
            for (group in [env.CODEDEPLOY_DISCOVERY_FRONTEND_DEPLOYMENT_GROUP, env.CODEDEPLOY_DISCOVERY_PUBLISHING_DEPLOYMENT_GROUP]) {
                sh sprintf('aws deploy create-deployment %s %s %s,bundleType=tgz,key=%s', [
                    '--application-name dp-dd-frontend-controller',
                    "--deployment-group-name ${group}",
                    "--s3-location bucket=${env.S3_REVISIONS_BUCKET}",
                    "dp-dd-frontend-controller-${revision}.tar.gz",
                ])
            }
        }
    }
